...
class GRUD(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, x_mean=0,
        ...
        self.zores = torch.autograd.Variable(torch.zeros(input_size))
    ...
    def forward(self, input):
        ...
        Hidden_State = torch.autograd.Variable(torch.zeros(self.hidden_size))
...
def fit(model, criterion, learning_rate, train_dataloader,
        dev_dataloader, test_dataloader, learning_rate_decay=0, n_epochs=30):
    epoch_losses = []
    # to check the update
    old_state_dict = {}
...
input_size = 1  # num of variables
hidden_size = 50  # same as inputsize
output_size = 1
num_layers = 1440  # num of step or layers base on the paper
